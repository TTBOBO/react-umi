	...
	time: {
		...
		*/
		...
		getNowTime: function (time, statu = false, str = '-') {
            ...
            let getMilliseconds = time.getSeconds() < 10 ? "0" + (time.getSeconds()) : time.getSeconds();
			...
			if (statu) {
				...
				time = (time.getFullYear() + str + month + str + data + " " + getHours + ":" + getMinutes + ":" + getMilliseconds);
			}
			return time;
		},
	...
	initValidate(option) {
		...
		option.valideDate.forEach((item) => {
			...
			if (verifyArr[item.validate] || typeof item.validate == 'object') {
				let varify = typeof item.validate === 'object' ?  item.validate : verifyArr[item.validate];
				validateNum = (rule, value, callback) => {
					if (!varify[0].test(value)) {
						callback(new Error(item.msg ? item.msg : varify[1]));   //返回验证错误信息
					} else {
						callback();
					}
				};
					...
					obj[item.field] = [
						{
							...
							trigger: item.type === 'select' || item.type === 'checkbox' ? "change" : "blur"
						}
					];
				}
			}else{
    ...
    throttle(func, wait, mustRun) {
        ...
        return function() {
            ...
            if (curTime - startTime >= mustRun) {
                ...
                timeout = setTimeout(func, wait,args && [...args][0]);
            }
        };
    },
